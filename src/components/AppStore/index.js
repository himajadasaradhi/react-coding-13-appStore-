...
class AppStore extends Component {
  ...
  render() {
    ...
    const filteredSearchItems = filteredItems.filter(eachItem =>
          ...
          <ul className="tab-flex">
            {tabsList.map(eachList => (
              <TabItem
                tabsList={eachList}
                key={eachList.tabId}
                onDisplayProjects={this.onDisplayProjects}
                isActive={activeTabId === eachList.tabId}
              />
            ))}
          </ul>
          <ul className="app-flex">
            {filteredSearchItems.map(eachList => (
              <AppItem appsList={eachList} key={eachList.appId} />
            ))}
          </ul>
        </div>
      </div>
